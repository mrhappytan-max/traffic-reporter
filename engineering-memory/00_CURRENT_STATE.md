...
| Known Blocker | **GOOGLE_DRIVE_SYNC_BLOCKED_FOR_NEW_FILES**（既知，見07_KNOWN_ISSUES_02.md，不阻擋封版）＋ **BUILD_METADATA_GENERATION_BUG**（`OPEN`，deployedCommit="unknown"，導致PRODUCTION_COMMIT_VERIFICATION=UNVERIFIABLE，獨立後續治理項）＋ 新增 **STALE_MEETING_ROOM_EXPORT**（`meeting-room-export/`自V2.4.4停更11個版本、曾提供TDX額度過期錯誤資訊，本輪已加註警告＋AGENTS.md入口改指向本目錄，`_history/`舊歷史搬遷仍待辦，詳見07_KNOWN_ISSUES_02.md）。另沿用 V2.4.5 封版的 REAL_WORLD_CONFIRMATION_PENDING（TDX正式LINE通知現場觀察） |

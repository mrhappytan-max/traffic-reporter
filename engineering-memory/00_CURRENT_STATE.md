...
| Current Version | V2.8.0（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-08 路況-064：播報/抓取時間窗由08:00~22:00調整為07:00~22:30（`broadcastHours.js#isWithinBroadcastHours()`改寫為分鐘數比較，TDX抓取與LINE/電報推播共用此函式同步生效），修正LINE Bot固定回覆文字（移除與真實行為不符的60分鐘forecast宣稱），查修頁新增「通知時段外」專屬顯示理由（`withinBroadcastHours`欄位新增並補上傳遞），執行路況-063規劃，MINOR bump（直接改變TDX抓取/LINE電報推播的真實決策時間窗，非純觀測性變更），詳見07_KNOWN_ISSUES_04.md）。**SEALED（2026-09-08，路況-064，依AGENTS.md第6節一段式封版規則）**。前八版V2.7.0（路況-061）、V2.6.1（路況-056）、V2.6.0（路況-055）、V2.5.1（路況-053）、V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md／07_KNOWN_ISSUES_04.md |
...
| Current Task | none。Latest completed task = 路況-064（BROADCAST_AND_TDX_TIME_WINDOW_07_2230_ADJUSTMENT，V2.8.0 Runtime變更：`broadcastHours.js#isWithinBroadcastHours()`改寫為07:00~22:30分鐘數比較、`line/webhook.js`固定回覆文字修正、`aiObservatoryIndex.js`新增`withinBroadcastHours`欄位與「通知時段外」查修頁分支，執行路況-063規劃）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE，本輪未動`pbsSchedule.js`本身（已停用legacy機制，非本次範圍）、AI Prompt/model、CCTV產圖邏輯、`incidentSuppression.js`、`sameIncident`/`materialChange`推播閘門邏輯（V2.7.0剛完成）、LINE/電報發送邏輯本身。V2.7.0及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.8.0為新版本線延續，不受該封版令限制。待驗證：07:00開始後第一則TDX/PBS事件是否確實被正確抓取與推播、22:30邊界是否正確生效，詳見07_KNOWN_ISSUES_04.md。 |
| Latest Completed Version | V2.8.0 |

...
| Known Blocker | ~~GOOGLE_DRIVE_SYNC_BLOCKED_FOR_NEW_FILES~~ **已決議退休（2026-09-07）**——Drive鏡像workflow（`.github/workflows/sync-engineering-memory.yml`）已停用push觸發，GitHub上的engineering-memory/為唯一正本，不再列為blocker，詳見07_KNOWN_ISSUES_02.md ＋ **BUILD_METADATA_GENERATION_BUG**（根因已確認2026-09-07：Cloudflare Build command為空值，未經npm predeploy hook，故deployedCommit永遠unknown；**修法未定案**，需真人改Dashboard設定，本輪未改，詳見07_KNOWN_ISSUES_02.md）＋ **STALE_MEETING_ROOM_EXPORT**（`meeting-room-export/`自V2.4.4停更11個版本、曾提供TDX額度過期錯誤資訊，已加註警告＋AGENTS.md入口改指向本目錄；`_history/`舊歷史原文已於2026-09-06原樣搬遷保存至`_history_archive/`，來源未刪除；`meeting-room-export/`最終停用仍為未結待辦，詳見07_KNOWN_ISSUES_02.md）＋ ~~PBS_RELAY_NO_OUTAGE_ALERTING~~ **已處置（2026-09-07）**——已建立HealthWatchdog排程（本機跳視窗警告，30分鐘輪詢health端點），正常路徑已驗證；`WATCHDOG_ALERT_PATH_UNVERIFIED`（異常路徑尚未實測跳窗）；限制：僅真人在電腦前/遠端連線時才看得到警告，長時間離機仍可能延遲察覺，詳見07_KNOWN_ISSUES_02.md。另沿用 V2.4.5 封版的 REAL_WORLD_CONFIRMATION_PENDING（TDX正式LINE通知現場觀察） |
...
| Next Action | ~~①滿24小時Production observation~~ **已完成（2026-09-07，見上方Real-world Confirmation）**。現行待辦：②BUILD_METADATA修法待定案（可能方向：調整Cloudflare Build command，屬Dashboard設定變更，需真人執行，本輪不改）；③禁止直接修改已封版V2.4.15，新問題一律開V2.4.16；④下次重開機後驗證LocalMonitor／Relay電源設定修正是否讓兩者自動啟動，PENDING；⑤~~純文件commit會觸發Worker重新部署，是否調整Build watch paths Exclude設定待決~~ **已執行（2026-09-07）**：Exclude已加入engineering-memory/**、meeting-room-export/**、*.md、AGENTS.md（原node_modules/**、.git/實為placeholder非已設定值，已一併補上），生效與否待本次push自然驗證，詳見07_KNOWN_ISSUES_02.md |

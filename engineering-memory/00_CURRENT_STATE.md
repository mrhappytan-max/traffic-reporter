...
| Current Version | V2.7.0（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-08 路況-061：讓AI的`sameIncident`/`materialChange`真正決定是否推播（`debugPush.js#runAiDecisionPath()`新增攔截區塊，`decision.notify && decision.sameIncident===true && decision.materialChange===false`時不推播），並修正查修頁`memoryContextFingerprint` join漏洞（`debugPush.js`/`aiObservatoryIndex.js`/`aiObservatoryView.js`三檔案），執行路況-060規劃，MINOR bump（直接改變AI核准事件是否推播的決策本身，非純觀測性變更），詳見07_KNOWN_ISSUES_04.md）。**SEALED（2026-09-08，路況-061，依AGENTS.md第6節一段式封版規則）**。前七版V2.6.1（路況-056）、V2.6.0（路況-055）、V2.5.1（路況-053）、V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md／07_KNOWN_ISSUES_04.md |
...
| Current Task | none。Latest completed task = 路況-061（AI_SAME_INCIDENT_MATERIAL_CHANGE_PUSH_GATE_AND_OBSERVATORY_JOIN_FIX，V2.7.0 Runtime變更：`debugPush.js#runAiDecisionPath()`新增AI判斷推播閘門＋三檔案修正Observatory查修頁`memoryContextFingerprint` join漏洞，執行路況-060規劃）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE，本輪未動AI Prompt/model、CCTV產圖邏輯、LINE或電報發送邏輯本身（`aiApprovedPbsBroadcast.js`／兩個`pushMessage.js`模組零異動）、`incidentSuppression.js`10分鐘collision window邏輯、`suppressLineNotify`（Phase B/C TDX閘門）邏輯。V2.6.1及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.7.0為新版本線延續，不受該封版令限制。待驗證：下一則同一事故無實質變化更新是否確實被正確攔截、10分鐘collision window實際觸發率下降的現場驗證，詳見07_KNOWN_ISSUES_04.md。 |
| Latest Completed Version | V2.7.0 |

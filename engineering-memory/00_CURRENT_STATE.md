...
| Current Version | V2.8.1（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-09 路況-066（取代路況-065）：查修頁展開卡片新增一鍵全選文字區塊，純CSS/HTML、零JavaScript、零CSP變更（`aiObservatoryView.js`新增`buildDetailPlainText()`/`renderCopyAllTextBlock()`/`detailSectionTitles()`），PATCH bump（純顯示層新增，不改變任何既有欄位計算方式或資料來源）。路況-065原授權按鈕點擊即以`navigator.clipboard.writeText()`複製，工程部查證後正確停下回報：此舉需放寬全Admin頁面共用CSP（`adminAuth.js`），超出授權範圍，真人裁示改採方案B（純CSS全選，使用者仍需自行Ctrl+C/長按複製），詳見07_KNOWN_ISSUES_04.md）。**SEALED（2026-09-09，路況-066，依AGENTS.md第6節一段式封版規則）**。前九版V2.8.0（路況-064）、V2.7.0（路況-061）、V2.6.1（路況-056）、V2.6.0（路況-055）、V2.5.1（路況-053）、V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md／07_KNOWN_ISSUES_04.md |
...
| Current Task | none。Latest completed task = 路況-066（取代路況-065，查修頁展開卡片新增一鍵全選文字區塊，V2.8.1 Runtime變更：`aiObservatoryView.js`新增`buildDetailPlainText()`純文字鏡像+`renderCopyAllTextBlock()`容器+`detailSectionTitles()`共用標題，純CSS`user-select: all`點選全選，零JavaScript、零CSP變更）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE，本輪僅動`aiObservatoryView.js`一個檔案，未動`src/security/adminAuth.js`或任何CSP設定、AI決策邏輯、CCTV產圖邏輯、LINE/電報發送邏輯、任何欄位計算方式或資料來源。V2.8.0及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.8.1為新版本線延續，不受該封版令限制。待驗證：真實Production查修頁上此容器在手機瀏覽器（iOS Safari長按選單/Android Chrome）的實際使用體驗、真人是否確認此功能達成原始需求，詳見07_KNOWN_ISSUES_04.md。 |
| Latest Completed Version | V2.8.1 |

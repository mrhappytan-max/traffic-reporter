...
| Current Version | V2.9.0（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-10 路況-070（執行路況-069規劃）：新增「一小時內同位置不重複推播」硬性規則，優先權高於V2.7.0的AI語意判斷——連續兩天真實案例（09-09追撞、09-10砂石車追撞）證明V2.7.0的sameIncident/materialChange機制運作正確，真人仍決定疊加此硬性限制（產品方向轉向，非V2.7.0有誤，該機制零行變動）。新增獨立模組`src/pbs/positionCooldown.js`（獨立KV key，純KM比對1公里，不沿用/修改incidentMemory.js／incidentSuppression.js既有函式常數），插入於`debugPush.js#runAiDecisionPath()`的`suppressForNoChange`之後、`suppressLineNotify`之前。MINOR bump（直接改變LINE/電報推播決策），詳見07_KNOWN_ISSUES_04.md）。**SEALED（2026-09-10，路況-070，依AGENTS.md第6節一段式封版規則）**。前十一版V2.8.2（路況-068）、V2.8.1（路況-066）、V2.8.0（路況-064）、V2.7.0（路況-061）、V2.6.1（路況-056）、V2.6.0（路況-055）、V2.5.1（路況-053）、V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md／07_KNOWN_ISSUES_04.md |
...
| Current Task | none。Latest completed task = 路況-070（執行路況-069規劃，新增「一小時內同位置不重複推播」硬性規則，V2.9.0 Runtime變更：新增`src/pbs/positionCooldown.js`獨立模組＋`debugPush.js#runAiDecisionPath()`插入攔截判斷＋`aiObservatoryIndex.js`新增`positionCooldownBlocked`欄位）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE。本輪未動AI Prompt/model、V2.7.0既有的sameIncident/materialChange判斷機制與`suppressForNoChange`區塊、`incidentMemory.js`的`proximityMatch()`、`incidentSuppression.js`既有函式/常數、CCTV/LINE/電報發送邏輯本身。V2.8.2及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.9.0為新版本線延續，不受該封版令限制。查修頁本輪未新增此規則的專屬顯示理由文字（`positionCooldownBlocked`已寫入記錄但未渲染），留待後續派工單。待驗證：下一次60分鐘內同位置事件是否正確攔截、下一次LOW→HIGH升級是否正確例外放行、本輪自訂的8小時記錄保留期限與60分鐘inclusive邊界是否符合真人實際期待，詳見07_KNOWN_ISSUES_04.md。 |
| Latest Completed Version | V2.9.0 |

...
| Current Version | V2.10.0（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-17 路況-074（執行路況-073規劃）：LINE主動事故推播正式退役（關開關、程式碼保留，可隨時復原）——真人確認Telegram已觀察數日、穩定、無額度問題後定案。新增`isLineNotifyEnabled()`（`aiApprovedPbsBroadcast.js`，polarity比照`isCctvImageEnabled()`「預設開啟」寫法），置於`deliverToLineTargets()`最開頭短路於KV讀取之前，`deliverToTelegram()`零行變動；`wrangler.jsonc`同輪新增`LINE_NOTIFY_ENABLED:"FALSE"`。範圍僅限主動事故推播，LINE Bot互動回覆功能不受影響。MINOR bump，詳見07_KNOWN_ISSUES_04.md）。**SEALED（2026-09-17，路況-074，依AGENTS.md第6節一段式封版規則）**。前十二版V2.9.0（路況-070）、V2.8.2（路況-068）、V2.8.1（路況-066）、V2.8.0（路況-064）、V2.7.0（路況-061）、V2.6.1（路況-056）、V2.6.0（路況-055）、V2.5.1（路況-053）、V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md／07_KNOWN_ISSUES_04.md |
...
| Current Task | none。Latest completed task = 路況-074（執行路況-073規劃，LINE主動事故推播正式退役，V2.10.0 Runtime變更：`aiApprovedPbsBroadcast.js`新增`isLineNotifyEnabled()`＋`deliverToLineTargets()`開頭短路＋`wrangler.jsonc`新增`LINE_NOTIFY_ENABLED:"FALSE"`）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE。本輪未動`deliverToTelegram()`、AI決策邏輯、CCTV產圖邏輯、V2.7.0/V2.9.0既有判斷邏輯本身、`line/webhook.js`/`line/replyMessage.js`、`line:subscriptions`名單（零寫入）、任何LINE相關程式碼/依賴/測試/Secret的刪除。V2.9.0及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.10.0為新版本線延續，不受該封版令限制。復原方式：僅需將`LINE_NOTIFY_ENABLED`改回`"true"`即可完整復原。待驗證：LINE群組（含「小黃多元計程車分享群」）靜默停止後是否有人反應異常、Telegram作為唯一主動推播管道是否持續穩定，詳見07_KNOWN_ISSUES_04.md。 |
| Latest Completed Version | V2.10.0 |

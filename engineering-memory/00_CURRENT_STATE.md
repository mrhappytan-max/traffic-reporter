...
> **Known Issues 已分卷（2026-09-04起，2026-09-07 增至三卷）**：`07_KNOWN_ISSUES.md`（Volume 01，歷史封存，完整保留不刪）＋`07_KNOWN_ISSUES_02.md`（Volume 02，歷史封存，完整保留不刪）＋`07_KNOWN_ISSUES_03.md`（Volume 03，CURRENT，新記錄從這裡寫）。查任何舊 Bug／技術債／根因／修復教訓，**三卷都要查**，只讀一、兩卷不算完整。詳見 Volume 03 開頭的卷別承接說明。
...
| Current Version | V2.5.1（唯一權威來源：`src/version.js` 的 `APP_VERSION`；2026-09-08 路況-053：CCTV完整診斷資訊（imageUrl／imageExpiresAt／cctvSkippedByReason／imageStrategy／r2ReadbackElapsedMs）接入AI Observatory index並顯示於查修頁，執行路況-046原始規劃，PATCH bump（比照V2.0.1純觀測性變更先例），詳見07_KNOWN_ISSUES_03.md）。**SEALED（2026-09-08，路況-053，依AGENTS.md第6節一段式封版規則）**。前四版V2.5.0（路況-052）、V2.4.18（路況-049）、V2.4.17（路況-044）、V2.4.16（路況-040）皆為SEALED，詳見07_KNOWN_ISSUES_03.md |
...
| Current Task | none。Latest completed task = 路況-053（AI_OBSERVATORY_CCTV_DIAGNOSTIC_FIELDS_INTEGRATION，V2.5.1 Runtime變更：`aiApprovedPbsBroadcast.js`／`debugPush.js`／`aiObservatoryIndex.js`／`aiObservatoryView.js`四檔案接入CCTV完整診斷5欄位，執行路況-046規劃、依V2.4.18現況重新核對情境數）。CURRENT_RUNTIME_PHASE 仍 PHASE_E_TDX_NOTIFY_LIVE，本輪未動AI決策邏輯／`LINE_PUSH_POLICY`／LINE或Telegram推播邏輯／CCTV產圖邏輯（`dynamicCollage.js`／`resolveCctvEligibility()`）。V2.5.0及更早版本封版規則對其本身仍有效（不得再改已封版記錄）；V2.5.1為新版本線延續，不受該封版令限制。待驗證：查修頁上5個新欄位在真實Production事件下的實際顯示效果，詳見07_KNOWN_ISSUES_03.md。 |
| Latest Completed Version | V2.5.1 |
...
- 逐版決策、根因、測試數字 → `07_KNOWN_ISSUES.md`（Volume 01）／`07_KNOWN_ISSUES_02.md`（Volume 02）／`07_KNOWN_ISSUES_03.md`（Volume 03）

...
## 治理紀錄｜工程部自行產出派工單編號衝突（2026-09-07）

**經過**：2026-09-07，真人在對話中直接請 Claude Code repo 工程部 session「做一個克勞德瀏覽器的施工單」，用途是交給另一個具瀏覽器能力、由真人在 Google Chrome 開啟的 Claude session 去查 Cloudflare Dashboard 數據。工程部 session 依此請求草擬了一份格式與既有派工單相同的文件，並沿用既有「路況-NNN」流水號續編為「路況-011」，但這份文件並非由 Chat 會議室發出——會議室自身另有一套獨立管理的編號序列，工程部這次自行續編因此與會議室的編號序列產生衝突。內容本身為唯讀查詢性質、未執行任何寫入或變更，但機制上工程部在此步驟自行決定了工作範圍、驗收欄位與編號，屬於原本應由會議室把關的事項。

**已採取的處置**：
1. `AGENTS.md` 新增「## 0. Task orders come only from the Meeting Room」規則段落（本輪，路況-012），明確規定派工單只能由 Chat 會議室產出，工程部（含 Claude Code repo 工程部與 Cowork 本機工程部）不得自行發單、不得自訂驗收標準；工程部若認為有必要的額外工作，正確做法是在回報中提出建議，由會議室評估決定，不得自行執行；會議室以外產生的派工單編號一律視為無效。
...
## 現況更新｜Cloudflare Workers 付費方案額度，舊有免費方案上限假設已失效（2026-09-07）

**來源聲明**：本節資訊由真人於 2026-09-07 提供，來自其 Cloudflare Dashboard 方案頁與官方定價文件。本 session 無 Cloudflare 存取權，未獨立驗證。

**1. 目前方案**：Cloudflare Workers 付費方案，每月 5 美元。性質為「訂閱費 + 用量計價」，非固定功能包。

**2. 方案含量與超額計價**：每月含 1,000 萬個請求，超過部分 $0.30／百萬；每月含 3,000 萬 CPU 毫秒，超過部分 $0.02／百萬；標準工單支援。

**3. 與免費方案的關鍵差異（免費 → 付費）**：
- 請求量：10 萬／天硬上限 → 每月含 1,000 萬，之後按量計費
- 單次 CPU 時間：10 毫秒 → 5 分鐘（預設 30 秒）
- Cron／Queue consumer CPU：10 毫秒 → 15 分鐘
- 每請求子請求數：50 → 10,000
- Cron 觸發器／帳戶：5 → 250
- Workers 數量：100 → 500
- 環境變數／Worker：64 → 128
- Workers Builds 建置分鐘：3,000／月 → 6,000／月
- 同時建置數：1 → 6
- KV 讀取：10 萬／天 → 按量計費，無日上限
- **KV 寫入／刪除／列出：1,000／天 → 按量計費，無日上限**
- Hyperdrive 查詢：10 萬／天 → 無限制
- Containers／Email Sending：不支援 → 已包含
- Workers Logs 事件：20 萬／天 → 按量計費

**4. 對既有工程記憶的影響**：工程記憶中多處設計分析係以「KV 每日寫入上限 1,000 次」為前提，最顯著者包括 V1.9.7 持久冪等層的 KV 成本量化（實測 128／148／218 writes/day，當時判定 `KV_WRITE_PRESSURE=LOW` 係相對於 1,000 上限而言）與 V2.2.0 的 `puts=4N+2` 成本計算（當時記為「200 events/day 為 802 puts/day，遠低於 Free Plan 每日 1,000 額度上限」）。**該上限現已不存在**，KV 寫入改為按量計費、無日上限。上述既有分析內容不改寫、完整保留為歷史記錄——當時的量測與推理過程本身正確且有價值，只是其外部前提已變。未來若重新評估相關設計取捨，應以本則記錄的現況為準，不得再以 1,000 次日上限作為約束條件。同理，其他以免費方案上限為前提的設計約束（子請求 50 個、單次 CPU 10 毫秒、Cron 觸發器 5 個等）亦已放寬。

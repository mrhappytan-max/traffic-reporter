...
**Chinese font approach — how it was chosen:** evaluated against the same constraints as V1.8's original ASCII font (no browser screenshot, no external image service, no Windows Relay, must not break the `@jsquash/jpeg` WASM setup, must not let bundle size/CPU run away) — a hand-authored bitmap font, exactly the same technique already proven for the ASCII glyphs, just extended to the **closed 24-character set this round's text actually needs**: 國附近監視畫面更新南前後北距事故公里無符合鏡頭暫. No CJK web font, no WASM SVG rasterizer, no new dependency of any kind.
- **Layout:** 16×16 ("full-width") for the 24 CJK glyphs, 8×16 ("half-width") for digits/K/+/:/ /./space — the standard full-width-CJK / half-width-Latin monospace convention, so mixed strings like `82K+900 / 距事故 0.800 公里` line up on one baseline. `src/cctv/bitmapFont.js`'s old 5×7 Latin-letter table is retired entirely (no longer needed — every display string this round is either digits/K/+/:/ / or one of the 24 CJK characters).
- **New glyph count:** 24 CJK + 16 half-width (digits 0-9, K, +, :, /, ., space) = 40 glyphs total.
- **Bundle impact:** `bitmapFont.js` grew from ~6.4KB to ~18.5KB (plain JS glyph data, no binary asset, no WASM) — negligible against Cloudflare Workers' compressed-bundle limits. Zero new npm dependencies, zero new WASM modules; the existing `@jsquash/jpeg` codec path (`jpegCodecWorker.js`) is completely untouched by this round.
- **Verification process — these are simplified dot-matrix-style glyphs, not calligraphy, so they were checked, not assumed:** authored in 3 rounds (a first draft, a full-legibility review pass rewriting the weakest ~10 glyphs, and a targeted 4-glyph touch-up), each round rendered to a real JPEG proof sheet and visually inspected before moving on. The final set was also rendered as the actual mixed-text strings this round produces (title, quadrant labels, the combined info line, both placeholders) and inspected in that real context — not just as isolated characters. `test/cctvCollage.test.js`'s test 10 automates the checkable half of this: every one of the 24 CJK characters renders at least one lit pixel (never a blank box) and no two characters render an identical bitmap (never visually confusable) — it cannot automate "does this look calligraphically correct," which is why the visual proof-sheet step existed at all.
- **Known limitation, accepted deliberately:** at 16×16, several of the busier characters (附/近/監/距/鏡/暫 in particular) are simplified enough that they wouldn't pass an isolated flash-card recognition test — but every string using them is one of a small number of fixed, repeated phrases in a driver's own familiar domain ("附近監視畫面," "距事故 … 公里," "無符合鏡頭"), which gives real-world recognition a large head start that an isolated-character test doesn't capture. If a future round needs stronger fidelity, a real font-rasterization pipeline is the correct next step, not more hand-tuning of this bitmap set — flag it explicitly if legibility complaints come back from real drivers.

**Header/label sizing — grew from V1.8, per instruction ("標題文字不要太小"):** the CJK/digit glyphs are inherently taller than the old ASCII font (16px vs. 7px), so `HEADER_HEIGHT` (80→100) and the per-cell `LABEL_HEIGHT` (90→110) both grew to keep the bigger text comfortably spaced; `IMAGE_AREA_HEIGHT` shrank correspondingly (320→290) to preserve the fixed 1200×900 canvas size. Title renders at scale 3 (48px-tall glyphs), subtitle and the per-cell info line at scale 2 (32px), quadrant labels at scale 3 (48px) — all comfortably legible on a phone screen in LINE.

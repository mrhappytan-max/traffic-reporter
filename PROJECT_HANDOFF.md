...
**V1.8.1 hard rule (added after real Production testing found a bad selection): 服務區／休息站／服務站 CCTV 不得用於主線事故判斷.** Live testing at 國1 82K+100 found the N後 quadrant selecting `86K+000` "北上湖口服務區" — a KM-proximity match, but a camera that can point at a parking lot, gas station, or the service area's own internal road, never guaranteed to show the freeway mainline at all. A nearby KM number does not mean mainline visibility.
...
- **LocationType's real semantics could not be verified.** TDX's live API and documentation are both unreachable from this development sandbox (network egress to `tdx.transportdata.tw` is blocked) — so this deliberately does **not** hardcode any `LocationType` enum-value assumption ("不要猜 LocationType 數值代表什麼"). Instead, `isServiceAreaCctv()` scans **every string-valued field on the raw TDX record** (`RoadSection`, `RoadName`, `LocationType` if it happens itself to be textual, or any other descriptive field) for the keywords `服務區`/`休息站`/`服務站` — a guess-free check grounded in whatever the field's own text actually says, that works regardless of which exact field name carries the description in the real payload, and doesn't silently stop working if TDX renames/adds a field. If a future round confirms from a real response that `LocationType` is a reliable numeric/enum service-area marker, prefer switching to that structured check instead — ask before doing so.
- **Deliberately narrow scope**, per instruction: only 服務區/休息站/服務站 this round. Interchanges (交流道), ramps (匝道), system interchanges (系統交流道), tunnels (隧道), and bridges (橋梁) are **not** excluded by this rule — whether those are appropriate for incident CCTV is a separate, not-yet-made decision.
...
- Zero impact on TDX call count (same single metadata response, filtered locally) or the four-quadrant rule's shape (S前/S後/N前/N後 order, ±2km→±4km→null strategy, max 4 cameras) — both unchanged. See `test/hsinchuCctvProbe.test.js`'s tests 2d–2g for the regression coverage (the real-world 湖口服務區 case, an all-service-area quadrant, proximity-never-overrides-exclusion, and cross-field keyword detection).

...
**Image compositing approach — how it was chosen:**
...
- **The actual WASM binaries are embedded as base64 string constants** (`src/cctv/generated/jpegWasmAssets.js`), decoded via `atob()` + `WebAssembly.compile()` at runtime (`src/cctv/jpegCodec.js`) — deliberately **not** a Cloudflare-Workers-style `import mod from './x.wasm'`. Two reasons: (1) this project's test suite runs under plain `node --test` with no ESM WASM-module loader, and every test file imports `src/index.js` at the top, so a top-level `.wasm` import anywhere in that module graph would break the *entire* suite, not just the new tests; (2) this sandbox has no way to live-verify wrangler's own `.wasm`-import bundling against a real Cloudflare deploy (no credentials, no egress to Cloudflare), whereas `WebAssembly.compile()`/`instantiate()` on raw bytes is standard, Workers-supported, and was fully exercised locally, so the **exact same code path runs in tests and in production** with zero environment-specific branching.
...
- **Bundle impact:** ~579KB of new source (dominated by the ~558KB base64-embedded WASM asset file, which gzips to ~168KB) plus ~80KB of `@jsquash/jpeg`'s own plain-JS emscripten glue — comfortably inside Cloudflare Workers' compressed-bundle limits on both Free and Paid plans.
- **CPU cost note (unverified in this sandbox — no live Workers execution available here):** decoding up to 4 small JPEGs, compositing, and re-encoding one 1200×900 JPEG is expected to be a low-double-digit-millisecond operation based on mozjpeg WASM's typical performance, but this project has no way to measure real Workers CPU time from this sandbox. Worth a quick real-world check after the first live deploy of this endpoint.

**A real bug found and fixed during this round, worth remembering:** `src/cctv/bitmapFont.js`'s `drawText` originally trusted its caller's `x`/`y` to already be integers. `src/cctv/collage.js`'s placeholder-tile label math (`y + h/2 - GLYPH_HEIGHT*1.5`) produces a value that always ends in `.5`. Writing to a `Uint8ClampedArray` at a **non-integer index is normally a silent no-op** — except that `idx = (py * canvasWidth + px) * 4` can land on a perfectly valid **integer** `idx` whenever `canvasWidth` is even (because `0.5 * canvasWidth` is then itself a whole number), which silently **wraps the write into a completely different, valid-looking row/column** instead of failing loudly or no-op'ing. This was caught by actually rendering test output to a JPEG and visually inspecting it (not by the pixel-sampling unit tests alone, which were written *after* the bug was found) — `drawText` now rounds `x`/`y` internally as defense-in-depth, and `test/cctvCollage.test.js`'s test 9 is a regression test for exactly this failure mode.

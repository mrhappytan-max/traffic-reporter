...
## 20. V1.8.6.4 — 省道 LINE message clarity + provenance audit

**Status: built and tested on `fix/v1.8.6.4-provincial-road-message-clarity`, NOT merged to `main`, NOT deployed.** Two parts: (1) a 省道 (any non-國1/國3 road, reported against 台3線) LINE message-clarity fix, (2) a follow-up root-cause **provenance audit** of that same fix, requested specifically to verify how much of the round's own reasoning was actually confirmed vs. assumed. This section documents the audit's findings — read this BEFORE trusting any "TDX confirmed X field" claim elsewhere in this file for RoadEvent-family location fields.
...
### Proposed (NOT implemented this round): a minimal broadcast provenance log

To answer "為什麼剛才這則長這樣" for a FUTURE event without ever needing to re-query TDX/PBS, this round designed (but did not build — a new pipeline touch-point, however small/best-effort, is left for explicit go-ahead) a short-TTL, Admin-only debug log:

- **New KV key shape**, modeled directly on `tdx/usageLedger.js`'s already-proven-safe append-only pattern (§18): `debug:broadcast-provenance:v1:<date>:<epochMs>:<opaqueId>`, one entry per event that was ACTUALLY pushed to at least one LINE target (not every eligible-but-unsent event — keeps volume bounded to what a person would actually ask "why did THIS look like that" about). TTL 48 hours (within the requested 24–72h range).
- **Entry contents** — every field already exists on the normalized event / already-computed pipeline result by the time `formatEventMessage` runs, so this is purely "write down what's already in memory," zero extra computation, zero extra TDX/PBS calls: `timestamp, source, rawId, type, road, direction, startKM, endKM, displayKM, location, locationDescription, eligibilityReason (from getBroadcastEligibility), formattedOutput (the exact LINE text sent)`. Deliberately excludes: any Secret/token, LINE userId/groupId, and the full raw TDX/PBS JSON response (only the already-extracted normalized fields + final text — never a "整份巨大 raw payload" dump).
- **Write site**: `broadcastPipeline.js`'s `runLineBroadcast`, immediately after a message is confirmed actually pushed (not before — never logs an event that didn't really go out), wrapped in the same best-effort try/catch isolation pattern `usageLedger.js` already uses (a KV outage here degrades to "provenance missing for this one event," never blocks/affects the real push, exactly like `test/tdxUsageLedger.test.js`'s test 16 proves for usage recording).
- **New read-only endpoint**: `GET /admin/broadcast-provenance` (Admin-Basic-Auth-gated, same convention as every other `/admin/*`/`/debug/*` route) — lists the last N entries, zero TDX/PBS/LINE calls, same guarantee as `/health`.
- **Explicitly untouched by this design**: `event.type`, broadcast eligibility (write happens strictly AFTER the decision, never feeds back into it), `notified.js`'s fingerprint (separate KV namespace entirely), Cron/TDX/PBS cadence, CCTV pipeline, quota dashboard/usage ledger.

Not implemented this round — proposed and ready to build on explicit approval.

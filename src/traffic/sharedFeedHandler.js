...
//
// The ONLY externally reachable surface of the Shared Traffic Feed. It is
// read-only by construction: it imports readSharedFeed/selectFeedWindow and
// has no access to any write path, so it cannot mutate producer state even
// by mistake.
//
// It also never fetches anything upstream. A KV miss returns an empty feed,
// never a re-fetch of TDX / PBS / CCTV — that guarantee is the whole reason
// the consuming project can claim "0 extra upstream calls".
//
// Transport is a Cloudflare Service Binding using HTTP-style fetch
...
// exact same code would serve an authenticated public HTTPS caller if the
// two Workers ever end up in different Cloudflare accounts.
...
export async function handleSharedFeed(request, env, now = new Date()) {
  ...
  if (!secret) {
    ...
    // distinguishable from a wrong token in logs.
  ...
  if (!feed.kvAvailable) {
    // A real storage outage. The consumer treats any non-200 as
    // "feed unavailable" and skips its round — it must never fall back to
    // an upstream source, so an honest error here is the correct answer.
